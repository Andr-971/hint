...
// ⁡⁢⁣⁢TYPE SCRIPT⁡
{
	...
	// ⁡⁢⁣⁣ЧАСТО ЗАДАВАЕМЫЕ ВОПРОСЫ НА СОБЕСЕДОВАНИИ⁡
	{
		...
		// ⁡⁣⁣⁢Декораторы в TypeScript — это специальные функции‑аннотации, позволяющие добавлять метаданные и модифицировать поведение классов, методов, свойств или параметров без изменения их исходного кода.⁡
		{
		...
		// ☑️ ⁡⁣⁢⁢ЧТО ТАКОЕ "УТИЛИТАРНЫЕ ТИПЫ"(UTILITY TYPES)(⁣Partial, ⁣Pick, ⁡⁣⁢⁣Omit)⁣Readonly,?⁡⁡
		...
		// ☑️ ⁡⁣⁢⁢СПЕЦИАЛЬНЫЕ ТИПЫ (any, unknown, never)⁡
		{
			// ⚡ ⁡⁣⁢⁡⁣⁢⁡⁢⁣⁣ЧТО ТАКОЕ ANY?⁡
			{
	...
	// 🔳 ⁡⁢⁣⁣СТРУКТУРА ТИПОВ⁡
	{
		...
		{
			// ⁡⁣⁣⁢Типы⁡
			// ⁡⁣⁣⁢├── Примитивные (string, number, boolean, bigint, symbol, undefined, null)⁡
			// ⁡⁣⁣⁢├── Объектные⁡
			// ⁡⁣⁣⁢│   ├── Объекты - type Person = { name: string; age: number; } interface Person {name: string; age: number;}⁡
			// ⁡⁣⁣⁢│   ├── Массивы - let numbers: number[] = [1, 2, 3]; или let numbers: Array<number> = [1, 2, 3];⁡
